...
# strictly what the code is doing, that is the purpose of the misspelling.
...
# This is the magic fn for troubleshooting only, shows up in output window
...
# updated if a new character type is added.  The "rg" prefix is
# important for determining if any character is selected.
def characterExists():
...
def setRun(self, context):
    cwmPanel.cycle = 4.0
    cwmPanel.bounce = 1.2
    cwmPanel.Hip_Rotate = 2.0
    cwmPanel.Sway_LR = 2.0   
    cwmPanel.Sway_FB = 4.0
    cwmPanel.Hip_UD = 3.0 
    cwmPanel.Shoulder_Rotation = 2.0
    cwmPanel.Shoulder_UD = 4.0 
    cwmPanel.Arm_Rotation = 4.0
    cwmPanel.rotatePositionHumerus = 0.0
    cwmPanel.rotateRangeHumerus = 1.0
    cwmPanel.rotatePositionUlna = 0.0
    cwmPanel.rotateRangeUlna = 1.0
    cwmPanel.rotatePositionFemur = 0.1   # was genProp. from here down
    cwmPanel.rotatePositionTibia = -.4 # 1.0
    cwmPanel.rotatePositionAnkle = 0.1
    cwmPanel.rotatePositionToe = -.1
    cwmPanel.rotateRangeFemur = 2.2
    cwmPanel.rotateRangeTibia = 1.0   # .6
    cwmPanel.rotateRangeAnkle = 2.2
    cwmPanel.rotateRangeToe = 1.8   # 2.2
    cwmPanel.rotateRangeBack = 1.0  # Need calibration
    cwmPanel.rotateRangeNeck = 1.0  # Need calibration
...
def setArmTwistL(self, context):
    ...
    rotate('armJ2.L', LArm_Twist, 1)
    rotate('armJ3.L', LArm_Twist, 1)
    rotate('armJ4.L', LArm_Twist, 1)
    rotate('armJ5.L', LArm_Twist, 1)
...
# These should eventually be sliders
def LHandOpenClose(self, context): # Light hand open - close
    name = getSelectedCharacterName()
    LHand_OC = cwmPanel.LHand_OC   * .1
    for i in range(1, 4):
        rotate('indexJ' + str(i) + '.L', LHand_OC, 2)
        rotate('midJ' + str(i) + '.L', LHand_OC, 2)
        rotate('ringJ' + str(i) + '.L', LHand_OC, 2)
        rotate('pinkyJ' + str(i) + '.L', LHand_OC, 2)
    for i in range(1, 4):
...
def RHandOpenClose(self, context): # Right hand open - close
    ...
    for i in range(1, 4):
        rotate('indexJ' + str(i) + '.R', RHand_OC, 2)
        rotate('midJ' + str(i) + '.R', RHand_OC, 2)
        rotate('ringJ' + str(i) + '.R', RHand_OC, 2)
        rotate('pinkyJ' + str(i) + '.R', RHand_OC, 2)
    for i in range(1, 4):
...
def setEye(self, context):
    ...
    rotate('eye.L', Eye_LR, 2)
    rotate('eye.R', Eye_LR, 2)
    #
    Eye_UD = cwmPanel.Eye_UD * .1   # Eye up - down
    rotate('eye.L', Eye_UD, 0)
    rotate('eye.R', Eye_UD, 0)
    bpy.context.object.data.bones['eye.L'].select  = True
    bpy.context.object.data.bones['eye.R'].select  = True
    bpy.ops.object.mode_set(mode='POSE')

# This will check what character the walk is to be applied to.
def setWalk(self, context):
    cwmPanel.cycle = 1.0
    cwmPanel.bounce = 1.2
    cwmPanel.Hip_Rotate = 0.0   # 2.0
    cwmPanel.Sway_LR = 1.0   
    cwmPanel.Sway_FB = 1.0
    cwmPanel.Hip_UD = 2.0 
    cwmPanel.Shoulder_Rotation = 1.2
    cwmPanel.Shoulder_UD = 1.0    # 4
    cwmPanel.Arm_Rotation = 3.0   # 4
    cwmPanel.rotatePositionHumerus = 0.0
    cwmPanel.rotateRangeHumerus = 1.0
    cwmPanel.rotatePositionUlna = 0.0
    cwmPanel.rotateRangeUlna = 1.0
    cwmPanel.rotatePositionFemur = 0.1   # was genProp from here down 
    cwmPanel.rotatePositionTibia = -.36
    cwmPanel.rotatePositionAnkle = 0.0
    cwmPanel.rotatePositionToe = -.1
    cwmPanel.rotateRangeFemur = 1.0
    cwmPanel.rotateRangeTibia = 1.0   # .6
    cwmPanel.rotateRangeAnkle = 1.0
    cwmPanel.rotateRangeToe = 1.0   
    cwmPanel.rotateRangeBack = 1.0  # Need calibration
    cwmPanel.rotateRangeNeck = 1.0  # Need calibration
    name = getSelectedCharacterName()
    if(name.startswith("rgbiped")):
        setCharacterAction(self, context)
    if(name.startswith("rgcentaur")):
        setCharacterAction(self, context)
    if(name.startswith("rgquadruped")):
        setCharacterAction(self, context)
    if(name.startswith("rgbird")):
        setBirdWalk(self, context)

def setBirdWalk(self, context):
    ob = bpy.context.object
    bpy.ops.object.mode_set(mode='OBJECT')
    name = getSelectedCharacterName()
    #
    ...
    RP = "+" + str(cwmPanel.rotatePositionFemur) + " " # Default = .1 which is added
    RR = str(cwmPanel.rotateRangeFemur) + " * " # Default = 1  which is multiplied
    ZeroAtFrame0 = getSpecialString(genProp.str_0AtFrame0)
    femurL = RR + "(" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('femurJ1.L'), femurL, 0, 'rotation_euler')
    femurR = RR + "(-1*" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('femurJ1.R'), femurR, 0, 'rotation_euler')
    # tibiaJ1
    RP = "+" + str(cwmPanel.rotatePositionTibia + .5) + " "
    RR = str(cwmPanel.rotateRangeTibia) + " * " # Default = 1  which is multiplied
    tibiaL = RR + "(-1*" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('tibiaJ1.L'), tibiaL, 0, 'rotation_euler')
    tibiaR = RR + "(" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('tibiaJ1.R'), tibiaR, 0, 'rotation_euler')
    # Ankles
    RP = "+" + str(cwmPanel.rotatePositionAnkle + -1) + " "
    RR = str(cwmPanel.rotateRangeAnkle) + " * " # Default = 1  which is multiplied
    ankleL = RR + "(" + 'clock()' + RP + ")*" + ZeroAtFrame0    
    setAxisDriver(getEuler('ankle.L'), ankleL, 0, 'rotation_euler')
    ankleR = RR + "(-1*" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('ankle.R'), ankleR, 0, 'rotation_euler')
    #
    '''
    ob = bpy.data.objects.get(name)        
    ob.select_set(True)
...
def toggleArmAction(self, context):
    if(genProp.toggleArmRotation == True):
        setArmRotation(self, context)
    else:
        unSetArmRotation(self, context)
    genProp.toggleArmRotation = not genProp.toggleArmRotation

def toggleLegAction(self, context):
    if(genProp.toggleLegRotation == True):
        setLegRotation(self, context)
    else:
        unSetLegRotation(self, context)
    genProp.toggleLegRotation = not genProp.toggleLegRotation

def setCharacterAction(self, context):
    ob = bpy.context.object
    bpy.ops.object.mode_set(mode='OBJECT')
    name = getSelectedCharacterName()
    #
    ...
    setAxisDriver(getEuler('femurJ1.R'), femurR, 0, 'rotation_euler')
    # tibiaJ1
    RP = "+" + str(cwmPanel.rotatePositionTibia) # + "-.26" # Adjusted
    RR = str(cwmPanel.rotateRangeTibia) + " * " #
    tibiaL = RR + "(" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('tibiaJ1.L'), tibiaL, 0, 'rotation_euler')
    tibiaR = RR + "(-1*" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('tibiaJ1.R'), tibiaR, 0, 'rotation_euler')
    # Ankles
    RP = "+" + str(cwmPanel.rotatePositionAnkle) + " "
    RR = str(cwmPanel.rotateRangeAnkle) + " * "
    ankleL = RR + "(-1*" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('ankle.L'), ankleL, 0, 'rotation_euler')
    ankleR = RR + "(" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('ankle.R'), ankleR, 0, 'rotation_euler')
    # Toes
    RP = "+" + str(cwmPanel.rotatePositionToe) + "+.26"
    RR = str(cwmPanel.rotateRangeToe) + " * " 
    toeL = RR + "(-1*" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('toe.L'), toeL, 0, 'rotation_euler')
    toeR = RR + "(" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('toe.R'), toeR, 0, 'rotation_euler')
    #
    # Add  movement for centaur rear legs
    if(name.startswith("rgcentaur") or name.startswith("rgquadruped")):
        ...
        # rearToes
        setAxisDriver(getEuler('rearToe.R'), toeL, 0, 'rotation_euler')
        setAxisDriver(getEuler('rearToe.L'), toeR, 0, 'rotation_euler')
        
    # Upper Back  # TODO - Quadruped needs neck movement set (fwd-backward)
    # A quadruped has no arms, and back and neck rotation so different from bipeds
    if(not name.startswith("rgquadruped")):
        BR = .4  # Back Rotate
        RRa = str(cwmPanel.rotateRangeBack*BR) + "*" # Adjusted
        ...
        RRa = str(cwmPanel.rotateRangeNeck * NR) + "*" # Adjusted
        fn = RRa + "(clock())*" + ZeroAtFrame0
        ...
        '''
        RR = "*" + str(1)
        fn = "(" + 'clock()' + RR + ")*" + ZeroAtFrame0
        setAxisDriver(getEuler('armJ1.L'), fn, 1)
        setAxisDriver(getEuler('armJ1.R'), fn, 1)
        # Elbows
        RR = ".5*"
        RP = "-.3"
        fn = RR + "(clock()" + RP + ")*" + ZeroAtFrame0
        setAxisDriver(getEuler('armJ3.L'), fn, 2)
        RP = "+.3"
        fn = RR + "(clock()" + RP + ")*" + ZeroAtFrame0
        setAxisDriver(getEuler('armJ3.R'), fn, 2)
        '''
        #
...
def setArmRotation(self, context):
    ZeroAtFrame0 = getSpecialString(genProp.str_0AtFrame0)
    Arm_Rotation = str(cwmPanel.Arm_Rotation)
    RR = "*" + str(cwmPanel.rotateRangeHumerus)
    RP = str(cwmPanel.rotatePositionHumerus) + "+"
    # Humerus - Arms rotation
    fn = "(" + RP + "clock()" + RR + ")*" + ZeroAtFrame0
    armJLDriver = setAxisDriver(getEuler('armJ1.L'), fn, 1)
    fn = "(" + RP + "clock()" + RR + ")*" + ZeroAtFrame0
    armJRDriver = setAxisDriver(getEuler('armJ1.R'), fn, 1)
    # Ulna - Elbows
    RR = "*" + str(cwmPanel.rotateRangeUlna)
    RP = str(cwmPanel.rotatePositionUlna) + "+"
    fn = "(-.4 + " + RP + "clock()" + RR + ")*" + ZeroAtFrame0
...
genProp.rotatePositionHumerus = FloatProperty(update=setArmRotation, name="Humerus_Rotation", default=0.0)
genProp.rotateRangeHumerus = FloatProperty(update=setArmRotation, name="Humerus_Range", default=1.0)
genProp.rotatePositionUlna = FloatProperty(update=setArmRotation, name="Ulna_Rotation", default=0.0)
genProp.rotateRangeUlna = FloatProperty(update=setArmRotation, name="Ulna_Range", default=1.0)
...
# 
genProp.rotatePositionFemur = FloatProperty(update=setCharacterAction, default=0.1)
genProp.rotatePositionTibia = FloatProperty(update=setCharacterAction, default= -.36)
genProp.rotatePositionAnkle = FloatProperty(update=setCharacterAction, default=0.1)
genProp.rotatePositionToe = FloatProperty(update=setCharacterAction, default= -0.1)
genProp.rotateRangeFemur = FloatProperty(update=setCharacterAction, default=1.0)
genProp.rotateRangeTibia = FloatProperty(update=setCharacterAction, default=1.0)
genProp.rotateRangeAnkle = FloatProperty(update=setCharacterAction, default=1.0)
genProp.rotateRangeToe = FloatProperty(update=setCharacterAction, default=1.0)
genProp.rotateRangeBack = FloatProperty(update=setCharacterAction, default=1.0)  # Need calibration
genProp.rotateRangeNeck = FloatProperty(update=setCharacterAction, default=1.0)  # Need calibration
genProp.tibiaJ1RP = FloatProperty(update=setCharacterAction, default=0.0)
genProp.tibiaJ1RR = FloatProperty(update=setCharacterAction, default=1.0)
genProp.ankleRR = FloatProperty(update=setCharacterAction, default=1.0)
genProp.toesRP = FloatProperty(update=setCharacterAction, default=-0.0)
genProp.toesRR = FloatProperty(update=setCharacterAction, default=1.0)
genProp.toggleArmRotation = BoolProperty(update=toggleArmAction, default=True)
genProp.toggleLegRotation = BoolProperty(update=toggleLegAction, default=True)
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
class Character(bpy.types.Operator):
    ...
    rotatePositionHumerus: FloatProperty(default=0.0)
    rotateRangeHumerus: FloatProperty(default=1.0)
    rotatePositionUlna: FloatProperty(default=0.0)
    rotateRangeUlna: FloatProperty(default=1.0)
    #
    ...
    X_Location = FloatProperty(update=update, default=0.0)
    Y_Location = FloatProperty(update=update, default=0.0)
    Z_Location = FloatProperty(update=update, default=1.18)
    ...
    # End  PopUp Window Properties declared strictly for class WM_OT_controol window
    rotatePositionFemur = FloatProperty(update=setCharacterAction, default=0.1)
    rotatePositionTibia = FloatProperty(update=setCharacterAction, default= -.36)
    rotatePositionAnkle = FloatProperty(update=setCharacterAction, default=0.1)
    rotatePositionToe = FloatProperty(update=setCharacterAction, default= -0.1)
    rotateRangeFemur = FloatProperty(update=setCharacterAction, default=1.0)
    rotateRangeTibia = FloatProperty(update=setCharacterAction, default=1.0)
    rotateRangeAnkle = FloatProperty(update=setCharacterAction, default=1.0)
    rotateRangeToe = FloatProperty(update=setCharacterAction, default=1.0)
    rotateRangeBack = FloatProperty(update=setCharacterAction, default=1.0)
    rotateRangeNeck = FloatProperty(update=setCharacterAction, default=1.0)
    # Sliders?
    ...
    toggleArmRotation = BoolProperty(toggleArmAction, default=True)
    toggleLegRotation = BoolProperty(toggleLegAction, default=True)
    #
    V_origin = (0, 0, 0)
...
class CHARACTER_OT_control_I(bpy.types.Operator):
    ...
    Sway_LR : FloatProperty(default=1.0)
    Sway_FB : FloatProperty(default=1.0)
    bounce : FloatProperty(default=1.2)
    Shoulder_Rotation : FloatProperty(default=1.2)  # 10
    Shoulder_UD : FloatProperty(default=1.0)  # 10
    Leg_Spread : FloatProperty(default=0.0)
    Hip_Sway : FloatProperty(default=5.0)
    Hip_Rotate : FloatProperty(default=0.0)
    Hip_UD : FloatProperty(default=2.0)
    Leg_Arch : FloatProperty(default=0.0)
    rotatePositionFemur : FloatProperty(default=0.1)
    rotatePositionTibia : FloatProperty(default= -.36)
    rotatePositionAnkle : FloatProperty(default=0.1)
    rotatePositionToe : FloatProperty(default= -0.1)
    rotateRangeFemur : FloatProperty(default=1.0)
    rotateRangeTibia : FloatProperty(default=1.0)
    rotateRangeAnkle : FloatProperty(default=1.0)
    rotateRangeToe : FloatProperty(default=1.0)
    #
    def execute(self, context): # This is all STATIC, not functions
        ob = bpy.context.object
        ...
        cwmPanel.rotatePositionFemur = self.rotatePositionFemur
        cwmPanel.rotatePositionTibia = self.rotatePositionTibia
        cwmPanel.rotatePositionAnkle = self.rotatePositionAnkle
        cwmPanel.rotatePositionToe = self.rotatePositionToe
        cwmPanel.rotateRangeFemur = self.rotateRangeFemur
        cwmPanel.rotateRangeTibia = self.rotateRangeTibia
        cwmPanel.rotateRangeAnkle = self.rotateRangeAnkle
        cwmPanel.rotateRangeToe = self.rotateRangeToe
        getSelectedCharacterName()
        bpy.ops.object.mode_set(mode='OBJECT')
        return {'FINISHED'}
    #
...
class CHARACTER_OT_control_II(bpy.types.Operator):
    ...
    #
    rotateRangeBack : FloatProperty(default=1.0)
    rotateRangeNeck : FloatProperty(default=1.0)
    Arm_Rotation : FloatProperty(default=3.0)
    rotatePositionHumerus : FloatProperty(default=0.0)
    rotateRangeHumerus : FloatProperty(default=1.0)
    rotatePositionUlna : FloatProperty(default=0.0)
    ...
    #toggleArmRotation : BoolProperty(default=True)
    #toggleLegRotation : BoolProperty(default=True)
    #
    def execute(self, context): # What is here is STATIC
        ob = bpy.context.object
        cwmPanel.rotateRangeBack = self.rotateRangeBack
        cwmPanel.rotateRangeNeck = self.rotateRangeNeck
        cwmPanel.Arm_Rotation = self.Arm_Rotation
        cwmPanel.rotatePositionHumerus = self.rotatePositionHumerus
        cwmPanel.rotateRangeHumerus = self.rotateRangeHumerus
        cwmPanel.rotatePositionUlna = self.rotatePositionUlna
        cwmPanel.rotateRangeUlna = self.rotateRangeUlna
        ...
        #cwmPanel.toggleArmRotation = self.toggleArmRotation
        #cwmPanel.toggleLegRotation = self.toggleLegRotation
        getSelectedCharacterName()
        bpy.ops.object.mode_set(mode='OBJECT')
        return {'FINISHED'}
    #
...
def dropArms(self, context):
    ob = bpy.context.object
    if(ob.name.startswith("rgbiped") or ob.name.startswith("rgcentaur")):
        rotate('armJ1.R', -90, 0)
        rotate('armJ1.L', -90, 0)
        bpy.ops.object.mode_set(mode='OBJECT')
...
#|
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#| What follows is build character functions and register functions
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#|
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
def buildQuadrupedSkeleton():
    ...
    char.armature.edit_bones['neckJ6.001'].name = "headBase"
    char.armature.edit_bones['neckJ6.002'].name = "eyeLevel"
    char.armature.edit_bones['neckJ6.003'].name = "headTop"
    char.armature.edit_bones['neckJ6.004'].name = "headFore"
    # ears
    bpy.ops.armature.select_all(action='DESELECT')
    char.armature.edit_bones["eyeLevel"].select_tail=True
    V_earRoot = (.05, -.06, .06)
    ...
    # jaw
    bpy.ops.armature.select_all(action='DESELECT')
    char.armature.edit_bones["neckJ6"].select_tail=True
    V_baseJaw = (0, .02, 0) 
    ...
    char.armature.edit_bones['neckJ6.001'].name = "baseJaw"
    char.armature.edit_bones['neckJ6.002'].name = "jaw"
    char.armature.edit_bones['neckJ6.002_R'].name = "jaw1.L"
    char.armature.edit_bones['neckJ6.002_L'].name = "jaw1.R"
    #
    # Eye Level
    bpy.ops.armature.select_all(action='DESELECT')
    char.armature.edit_bones["eyeLevel"].select_tail=True
