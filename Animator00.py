...
def setRun(self, context):
    ...
    #genProp.ccycle = 12    #genProp.Hip_Rotate = 2.0
    #genProp.Sway_LR = 2.0   
    #genProp.Sway_FB = 4.0
    #genProp.bounce = 1.2
    #genProp.Hip_UD = 2.0 
...
def revertAdvancedControls(self, context):
    #genProp.rooootatePosition = 0.1  # TODO
...
class Character(bpy.types.Operator):
    ...
    name = ""
    bone = ""
    at_name = "" # rgbiped01_at for troubleshooting
    x = 0
    y = 0
    z = 0
    n = 0
    ...
    pelvis = "pelvis"
    #
    # These are only for reset, not for creation,the exist already
    #V_hip = (0.12, 0, 0)
    #V_femurJ1 = (0, 0, -.18)
    #V_femurJ2 = (0, 0, -.16)
    #V_tibiaJ1 = (0, 0, -.1)
    ...
    #V_qBackJ4 = (0, -.12, .02)
    #V_qBackJ5 = (0, -.14, .01)
    V_fixRib1Top = (.1, 0, -.04)  # Removed the q
    ...
    V_hRumpfix3 = (0, .15, -.07)
    hRumpfix3 = "hRumpfix3"
    #V_hRumpfix3 = (0, .14, .05) This appears to be a reset
    V_hRumpfix4 = (0, .2, .04)
    hRumpfix4 = "hRumpfix4"
    #
    V_rearHip = (.16, 0, 0)
...
class CHARACTER_OT_control_II(bpy.types.Operator):
    ...
    def execute(self, context): # What is here is STATIC
        ...
        char.Arm_Rotation = self.Arm_Rotation
...
def buildRootArmature(char, charName="Alf", x=0, y=0, z=0):
    char.armature = bpy.data.armatures.new(charName + '_at')  # at = Armature
    char.at_name = char.armature.name
...
def setHandle(at, charName, Vhead, Vtail):
    bpy.ops.object.editmode_toggle()
    bone = at.edit_bones.new(charName + '_bone')
    bone.head = Vtail
    bone.tail = Vhead
    return bone

def boneMirror(armature, vector, mirror = False):
    bpy.data.armatures[armature].use_mirror_x = mirror
...
def setEnvelopeWeights(char):
    ...
    #
        ...
        if(char.name.startswith("rgbiped")) or (name.startswith("rgcentaur")):
...
def buildBipedSkeleton():
    char.n = getSceneObjectNumber() #Each character name is numbered sequentially.
    char.name = setName('biped', char.n)
    char.bone = char.name + "_bone"
    mod = fmod(char.n, 2)
    if(mod == 0):
        char.x = 1.0 * char.n - mod    # Can you build an armature at the origin
    else:                    # then start building bones independent of it?
        char.x = 1.0 * -char.n - mod
    #
    char.y = -.4 * char.n; char.z = 1.2  # y = Head of handle (Armature point)
    char.at = buildRootArmature(char, char.name, char.x, char.y, char.z) #creates point 
    # %%% FIRST BONE %%% FIRST CHARACTER WILL BE NAMED rgbiped01
    char.VHead = [0, 0, 0]
    char.Vtail = [0, -.3, 0] # Tail of Handle  
    bone = setHandle(char.at, char.name, char.VHead, char.Vtail)
    bpy.data.objects[char.name].show_in_front = True
    #
    char.V_angle = (0, 0, 0) # Bottom (Tail)
    char.angle = boneMirror(char.at_name, char.V_angle, False)
    char.V_hipC = (0, .1, -.1)
    boneMirror(char.at_name, char.V_hipC, False)
    #
    #*** Active bone is NOT right above, but next bone above ***
    bone_name = getActiveBoneName()
    char.at.edit_bones[bone_name].name = "angle"
    #
    boneMirror(char.at_name, char.V_next, False)
    bone_name = getActiveBoneName()
    char.at.edit_bones[bone_name].name = "hipC"
    # start mirroring! Can't use naming above on mirrored bones
    boneMirror(char.at_name, char.V_hip, True)
    boneMirror(char.at_name, char.V_femurJ1, True)
    boneMirror(char.at_name, char.V_femurJ2, True)
    boneMirror(char.at_name, char.V_tibiaJ1, True)
    boneMirror(char.at_name, char.V_tibiaJ2, True)
    boneMirror(char.at_name, char.V_ankle, True)
    boneMirror(char.at_name, char.V_toe, True)
    char.at.edit_bones["hipC_L"].name = "hip.R"
    char.at.edit_bones['hipC_R'].name = "hip.L"
    char.at.edit_bones['hipC_R.001'].name = "femurJ1.L"
    char.at.edit_bones['hipC_L.001'].name = "femurJ1.R"
    char.at.edit_bones['hipC_R.002'].name = "femurJ2.L"
    char.at.edit_bones['hipC_L.002'].name = "femurJ2.R"
    char.at.edit_bones['hipC_R.003'].name = "tibiaJ1.L"
    char.at.edit_bones['hipC_L.003'].name = "tibiaJ1.R"    
    char.at.edit_bones['hipC_R.004'].name = "tibiaJ2.L"
    char.at.edit_bones['hipC_L.004'].name = "tibiaJ2.R"    
    char.at.edit_bones['hipC_R.005'].name = "ankle.L"
    char.at.edit_bones['hipC_L.005'].name = "ankle.R"
    char.at.edit_bones['hipC_R.006'].name = "toe.L"
    char.at.edit_bones['hipC_L.006'].name = "toe.R"
    #
    # Heel starts were the tibia ends, same place as the ankle bone
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["ankle.L"].select_tail=True
    char.at.edit_bones["ankle.R"].select_tail=True
    boneMirror(char.at_name, char.V_heel, True)
    char.at.edit_bones['ankle.L.001'].name = "heel.L"
    char.at.edit_bones['ankle.R.001'].name = "heel.R"
    #
    # Reinforce rear
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["angle"].select_tail=True
    boneMirror(char.at_name, char.V_posterior, False)
    boneMirror(char.at_name, char.V_posterior1, True)
    boneMirror(char.at_name, char.V_posterior2, True)
    #
    bpy.ops.armature.select_all(action='DESELECT')
    selectBonePartByName("angle.001", True) # Tail=True
    V_tailBone = (0, 0, -.14)
    boneMirror(char.at_name, char.V_tailBone, False)
    char.at.edit_bones['angle.001'].name = "Posterior1"
    char.at.edit_bones['angle.001_L'].name = "fixPosterior1.L"
    char.at.edit_bones['angle.001_R'].name = "fixPosterior1.R"
    char.at.edit_bones['angle.001_L.001'].name = "fixPosterior2.L"
    char.at.edit_bones['angle.001_R.001'].name = "fixPosterior2.R"
    char.at.edit_bones['angle.002'].name = "fixPosterior2.C"
    #
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
def buildHumanUpperBody(char, boneName="angle", V_bBackJ1 = (0, 0, 0.1)):
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones[boneName].select_tail=True
    boneMirror(char.at_name, char.V_bBackJ1, False)
    boneMirror(char.at_name, char.V_bBackJ2, False)
    boneMirror(char.at_name, char.V_bBackJ3, False)
    boneMirror(char.at_name, char.V_bBackJ4, False)
    boneMirror(char.at_name, char.V_bBackJ5, False)
    char.at.edit_bones[boneName + ".001"].name = "bBackJ1"
    char.at.edit_bones[boneName + ".002"].name = "bBackJ2"
    char.at.edit_bones[boneName + ".003"].name = "bBackJ3"
    char.at.edit_bones[boneName + ".004"].name = "bBackJ4"
    char.at.edit_bones[boneName + ".005"].name = "bBackJ5"
    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    # Start arms  
    # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    # Shoulder area  
    boneMirror(char.at_name, char.V_clavicle, True)
    boneMirror(char.at_name, char.V_shoulder, True)
    boneMirror(char.at_name, char.V_armJ1, True)
    boneMirror(char.at_name, char.V_armJ2, True)
    boneMirror(char.at_name, char.V_armJ3, True)
    boneMirror(char.at_name, char.V_armJ4, True)
    boneMirror(char.at_name, char.V_armJ5, True)
    char.at.edit_bones['bBackJ5_L'].name = "clavicle.R"
    char.at.edit_bones['bBackJ5_R'].name = "clavicle.L"
    char.at.edit_bones['bBackJ5_L.001'].name = "shoulder.R"
    char.at.edit_bones['bBackJ5_R.001'].name = "shoulder.L"
    char.at.edit_bones['bBackJ5_L.002'].name = "armJ1.R"
    char.at.edit_bones['bBackJ5_R.002'].name = "armJ1.L"
    char.at.edit_bones['bBackJ5_L.003'].name = "armJ2.R"
    char.at.edit_bones['bBackJ5_R.003'].name = "armJ2.L"
    char.at.edit_bones['bBackJ5_L.004'].name = "armJ3.R"
    char.at.edit_bones['bBackJ5_R.004'].name = "armJ3.L"
    char.at.edit_bones['bBackJ5_L.005'].name = "armJ4.R"
    char.at.edit_bones['bBackJ5_R.005'].name = "armJ4.L"
    char.at.edit_bones['bBackJ5_L.006'].name = "armJ5.R"
    char.at.edit_bones['bBackJ5_R.006'].name = "armJ5.L"
    # Middle finger  
    boneMirror(char.at_name, char.V_wristBase2, True)
    boneMirror(char.at_name, char.V_midJ1, True)
    boneMirror(char.at_name, char.V_midJ2, True)
    boneMirror(char.at_name, char.V_midJ3, True)
    char.at.edit_bones['armJ5.L.001'].name = "wristBase2.L"
    char.at.edit_bones['armJ5.R.001'].name = "wristBase2.R"
    char.at.edit_bones['armJ5.L.002'].name = "midJ1.L"
    char.at.edit_bones['armJ5.R.002'].name = "midJ1.R"
    char.at.edit_bones['armJ5.L.003'].name = "midJ2.L"
    char.at.edit_bones['armJ5.R.003'].name = "midJ2.R"
    char.at.edit_bones['armJ5.L.004'].name = "midJ3.L"
    char.at.edit_bones['armJ5.R.004'].name = "midJ3.R"
    # Shift back to wrist
    # Index Finger bones   
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["armJ5.R"].select_tail=True
    boneMirror(char.at_name, char.V_wristBase1, True)
    boneMirror(char.at_name, char.V_indexJ1, True)
    boneMirror(char.at_name, char.V_indexJ2, True)
    boneMirror(char.at_name, char.V_indexJ3, True)
    char.at.edit_bones['armJ5.L.001'].name = "wristBase1.L"
    char.at.edit_bones['armJ5.R.001'].name = "wristBase1.R"
    char.at.edit_bones['armJ5.L.002'].name = "indexJ1.L"
    char.at.edit_bones['armJ5.R.002'].name = "indexJ1.R"
    char.at.edit_bones['armJ5.L.003'].name = "indexJ2.L"
    char.at.edit_bones['armJ5.R.003'].name = "indexJ2.R"
    char.at.edit_bones['armJ5.L.004'].name = "indexJ3.L"
    char.at.edit_bones['armJ5.R.004'].name = "indexJ3.R"
    #
    # Shift back to wrist
    # Ring Finger bones
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["armJ5.R"].select_tail=True
    boneMirror(char.at_name, char.V_wristBase3, True)
    boneMirror(char.at_name, char.V_ringJ1, True)
    boneMirror(char.at_name, char.V_ringJ2, True)
    boneMirror(char.at_name, char.V_ringJ3, True)
    char.at.edit_bones['armJ5.L.001'].name = "wristBase3.L"
    char.at.edit_bones['armJ5.R.001'].name = "wristBase3.R"
    char.at.edit_bones['armJ5.L.002'].name = "ringJ1.L"
    char.at.edit_bones['armJ5.R.002'].name = "ringJ1.R"
    char.at.edit_bones['armJ5.L.003'].name = "ringJ2.L"
    char.at.edit_bones['armJ5.R.003'].name = "ringJ2.R"
    char.at.edit_bones['armJ5.L.004'].name = "ringJ3.L"
    char.at.edit_bones['armJ5.R.004'].name = "ringJ3.R"
    # Shift back to wrist
    # Pinky Finger bones  
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["armJ5.R"].select_tail=True
    boneMirror(char.at_name, char.V_pinkyBase, True)
    boneMirror(char.at_name, char.V_pinkyJ1, True)
    boneMirror(char.at_name, char.V_pinkyJ2, True)
    boneMirror(char.at_name, char.V_pinkyJ3, True)
    char.at.edit_bones['armJ5.L.001'].name = "wristBase4.L"
    char.at.edit_bones['armJ5.R.001'].name = "wristBase4.R"
    char.at.edit_bones['armJ5.L.002'].name = "pinkyJ1.L"
    char.at.edit_bones['armJ5.R.002'].name = "pinkyJ1.R"
    char.at.edit_bones['armJ5.L.003'].name = "pinkyJ2.L"
    char.at.edit_bones['armJ5.R.003'].name = "pinkyJ2.R"
    char.at.edit_bones['armJ5.L.004'].name = "pinkyJ3.L"
    char.at.edit_bones['armJ5.R.004'].name = "pinkyJ3.R"
    #
    # Shift back to wrist
    # Thumb Finger bones
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["armJ5.R"].select_tail=True
    boneMirror(char.at_name, char.V_thumbBase, True)
    boneMirror(char.at_name, char.V_thumbJ1, True)
    boneMirror(char.at_name, char.V_thumbJ2, True)
    boneMirror(char.at_name, char.V_thumbJ3, True)
    char.at.edit_bones['armJ5.L.001'].name = "thumbBase.L"
    char.at.edit_bones['armJ5.R.001'].name = "thumbBase.R"
    char.at.edit_bones['armJ5.L.002'].name = "thumbJ1.L"
    char.at.edit_bones['armJ5.R.002'].name = "thumbJ1.R"
    char.at.edit_bones['armJ5.L.003'].name = "thumbJ2.L"
    char.at.edit_bones['armJ5.R.003'].name = "thumbJ2.R"
    char.at.edit_bones['armJ5.L.004'].name = "thumbJ3.L"
    char.at.edit_bones['armJ5.R.004'].name = "thumbJ3.R"
    # End arm and hand creation
    #
    # Resume spine
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["bBackJ5"].select_tail=True
    boneMirror(char.at_name, char.V_neckJ1, False)
    boneMirror(char.at_name, char.V_neckJ2, False)
    boneMirror(char.at_name, char.V_neckJ3, False)
    boneMirror(char.at_name, char.V_neckJ4, False)
    boneMirror(char.at_name, char.V_headBase, False)
    boneMirror(char.at_name, char.V_eyeLevel, False)
    boneMirror(char.at_name, char.V_headTop, False)
    boneMirror(char.at_name, char.V_headFore, False)
    char.at.edit_bones['bBackJ5.001'].name = "neckJ1"
    char.at.edit_bones['bBackJ5.002'].name = "neckJ2"
    char.at.edit_bones['bBackJ5.003'].name = "neckJ3"
    char.at.edit_bones['bBackJ5.004'].name = "neckJ4"
    char.at.edit_bones['bBackJ5.005'].name = "headBase"
    char.at.edit_bones['bBackJ5.006'].name = "eyeLevel"
    char.at.edit_bones['bBackJ5.007'].name = "headTop"
    char.at.edit_bones['bBackJ5.008'].name = "headFore"
    # jaw
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["neckJ4"].select_tail=True
    boneMirror(char.at_name, char.V_jaw, False)
    boneMirror(char.at_name, char.V_jaw1, True)
    boneMirror(char.at_name, char.V_jaw2, True)
    char.at.edit_bones['neckJ4.001'].name = "jaw"
    char.at.edit_bones['neckJ4.001_R'].name = "jaw1.L"
    char.at.edit_bones['neckJ4.001_L'].name = "jaw1.R"
    char.at.edit_bones['neckJ4.001_R.001'].name = "jaw2.L"
    char.at.edit_bones['neckJ4.001_L.001'].name = "jaw2.R"
    # upperMouth
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["neckJ4"].select_tail=True
    boneMirror(char.at_name, char.V_baseMouth1, False)
    boneMirror(char.at_name, char.V_baseMouth2, True)
    boneMirror(char.at_name, char.V_baseMouth3, True)
    char.at.edit_bones['neckJ4.001'].name = "baseMouth1"
    char.at.edit_bones['neckJ4.001_R'].name = "baseMouth2.L"
    char.at.edit_bones['neckJ4.001_L'].name = "baseMouth2.R"
    char.at.edit_bones['neckJ4.001_R.001'].name = "baseMouth3.L"
    char.at.edit_bones['neckJ4.001_L.001'].name = "baseMouth3.R"
    #
    # Eye Level
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["eyeLevel"].select_tail=True
    boneMirror(char.at_name, char.V_eyebase, True)
    boneMirror(char.at_name, char.V_eye, True)
    char.at.edit_bones['eyeLevel_L'].name = "eyeBase.L"
    char.at.edit_bones['eyeLevel_R'].name = "eyeBase.R"
    char.at.edit_bones['eyeLevel_L.001'].name = "eye.L"
    char.at.edit_bones['eyeLevel_R.001'].name = "eye.R"
    # nose
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["headBase"].select_tail=True
    boneMirror(char.at_name, char.V_noseBase, False)
    boneMirror(char.at_name, char.V_nose, False)
    char.at.edit_bones['headBase.001'].name = "noseBase"
    char.at.edit_bones['headBase.002'].name = "nose"
    #
    # Add front fix
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["bBackJ5"].select_tail=True
    boneMirror(char.at_name, char.V_fixChestFront, False)    
    boneMirror(char.at_name, char.V_fixChestFront1, True)
    boneMirror(char.at_name, char.V_fixChestFront2, True)
    char.at.edit_bones['bBackJ5.001'].name = "fixChestFront"
    char.at.edit_bones['bBackJ5.001_R'].name = "fixChestFront1.L"
    char.at.edit_bones['bBackJ5.001_L'].name = "fixChestFront1.R"
    char.at.edit_bones['bBackJ5.001_R.001'].name = "fixChestFront2.L"
    char.at.edit_bones['bBackJ5.001_L.001'].name = "fixChestFront2.R"
    # Add center fix
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["bBackJ5"].select_tail=True
    boneMirror(char.at_name, char.V_fixChestCenter1, True)
    boneMirror(char.at_name, char.V_fixChestCenter2, True)
    char.at.edit_bones['bBackJ5_R'].name = "fixChestCenter1.L"
    char.at.edit_bones['bBackJ5_L'].name = "fixChestCenter1.R"
    char.at.edit_bones['bBackJ5_R.001'].name = "fixChestCenter2.L"
    char.at.edit_bones['bBackJ5_L.001'].name = "fixChestCenter2.R"
    # Add rear fix
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["bBackJ5"].select_head=True
    boneMirror(char.at_name, char.V_fixChestBack, False)    
    boneMirror(char.at_name, char.V_fixChestBack1, True)
    boneMirror(char.at_name, char.V_fixChestBack2, True)
    char.at.edit_bones['bBackJ5.001'].name = "fixChestBack"
    char.at.edit_bones['bBackJ5.001_R'].name = "fixChestBack1.L"
    char.at.edit_bones['bBackJ5.001_L'].name = "fixChestBack1.R"
    char.at.edit_bones['bBackJ5.001_R.001'].name = "fixChestBack2.L"
    char.at.edit_bones['bBackJ5.001_L.001'].name = "fixChestBack2.R"
    #
    # Upper shoulder
    bpy.ops.armature.select_all(action='DESELECT')
    char.at.edit_bones["bBackJ5"].select_tail=True
    boneMirror(char.at_name, char.V_fixUpperShoulder1, False)
    boneMirror(char.at_name, char.V_fixUpperShoulder2, True)
    char.at.edit_bones['bBackJ5.001'].name = "fixShoulderBack.L"
    char.at.edit_bones['bBackJ5.001_R'].name = "fixShoulderBack1.L"
    char.at.edit_bones['bBackJ5.001_L'].name = "fixShoulderBack1.R"
