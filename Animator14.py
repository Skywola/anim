...
def revertAdvancedControls(self, context):
    genProp.femurJ1RP = 0.0
    genProp.femurJ1RR = 1.0
...
def setBipedWalk(self, context):
    ...
    femurJ1RP = str(cwmPanel.femurJ1RP) # TODO feemurJ1RP is not used, why not?
    femurJ1RR = str(cwmPanel.femurJ1RR) # TODO feemurJ1RR is not used, why not?
    femurJ1PP = "0.0" # Static pose position
    #
    RP = "+" + str(.1)
    ZeroAtFrame0 = getSpecialString(genProp.str_0AtFrame0)
    fnL = "(" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('femurJ1.L'), fnL, 0, 'rotation_euler')
    fnR = "(-1*" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('femurJ1.R'), fnR, 0, 'rotation_euler')
    # tibiaJ1
    tibiaJ1RP = str(cwmPanel.tibiaJ1RP) # TODO tiibiaJ1RP is not used, why not?
    tibiaJ1RR = str(cwmPanel.tibiaJ1RR) # TODO tiibiaJ1RR is not used, why not?
    RP = "-" + str(.4)
    fnL = "(" + 'clock()' + RP + ")*" + ZeroAtFrame0
    ...
    # Ankles
    ankleRP = str(cwmPanel.ankleRP) # TODO ankleeRP is not used, why not?
    ankleRR = str(cwmPanel.ankleRR) # TODO ankleeRR is not used, why not?
    RP = "+" + str(.3)
    fnL = "(-1*" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('ankle.L'), fnL, 0, 'rotation_euler')
    fnR = "(" + 'clock()' + RP + ")*" + ZeroAtFrame0
    setAxisDriver(getEuler('ankle.R'), fnR, 0, 'rotation_euler')
    # Toes
    toesRP = str(cwmPanel.toesRP) # TODO toeesRP is not used, why not?
    toesRR = str(cwmPanel.toesRR) # TODO toeesRR is not used, why not?
    RP = "+" + str(.1)
    fnL = "(-1*" + 'clock()' + RP + ")*" + ZeroAtFrame0
...
genProp.femurJ1RP = bpy.props.FloatProperty(update=setBipedWalk, name="femurJ1RP", default=0.0)
genProp.femurJ1RR = bpy.props.FloatProperty(update=setBipedWalk, name="femurJ1RR", default=1.0)
...
class WM_OT_control_I(bpy.types.Operator):
    ...
    def execute(self, context): # What is here is STATIC
        ...
        cwmPanel.Z_Location = self.Z_Location
