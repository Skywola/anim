...
def clock(freq=9, amp=.5):  # Sets the pace
    cycle = bpy.context.window_manager.cycle
...
def setHorizontalSpeed():
    spd = bpy.context.window_manager.speed
    frame = bpy.context.scene.frame_current
    spd = frame * .04 * spd
    return round(spd, 2)
...
def buildRootArmature(type, strCharName, x, y, z):
    ...
    type.x = x
    type.y = y
    type.rig.location = (x, y, z)  # Set armature point location
    # Link to scene
...
class UtilsFns:
    ...
    xlocation = 0.0 
    ylocation = 0.0
    # zlocation = 1.2 # Removed 6-26-2020 replaced by bipedHeight
    swayLR = 0.0
    swayFB = 1.0
    bounce = 2.0
    headUD = 0.0
    ...
    rearToesPP = 0
    click = False
    str0AtFrame0 = "*(frame * (1/(frame+.0001)))" #Zero at frame 0, otherwise a one
    str1AtFrame0 = "*abs((frame/(frame + .0001))-1)"# One at frame zero, otherwise a zero
    ...
    def setLocation(self, context):
        ...
        fn = "bpy.context.object.location.x = setHorizontalSpeed()"
        setDriver(getEuler(u.strName+"_bone"), fn, 0)
        u.ylocation = math.radians(bpy.context.window_manager.ylocation)
        bpy.data.objects[u.strName].location.y = u.ylocation
        bpy.data.objects[u.strName].location.z = u.bipedHeight
		# Was u.bipedHeight + u.bipedHeight above 6-26-2020
        #ob.location.x = u.xlocation
        #ob.location.y = u.ylocation
        #ob.location.z = u.bipedHeight
    #
...
# This could likely take the two effective lines, and put it under
# the panel function, and it would work.
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
def addCharactersProperties(self, context):
    ...
    for obj in bpy.context.scene.objects:
        if(not hasattr(winMan, "blank")):  # now frame (current frame)
            winMan.blank = bpy.props.FloatProperty(name="blank", default=0)
        if(not hasattr(winMan, "cycle")):
            winMan.cycle = bpy.props.FloatProperty(update=u.update, name="cycle", default=1.0)
        if(not hasattr(winMan, "speed")):
            winMan.speed = bpy.props.FloatProperty(update=u.update, name="speed")
        ...
        #if(not hasattr(winMan, "zlocation")): # Removed 6-26-2020 replacement is u.bipedHeight
        ...
        # Diverge Point - character differences between characters starts here
...
class WM_OT_control(bpy.types.Operator):
    ...
    #blank : bpy.props.IntProperty(name= "blank:")
    #
    def execute(self, context): # What is here is STATIC, no functions work
        u = UtilsFns
        ob = u.object = bpy.context.object
        bpy.context.window_manager.speed = self.speed
		bpy.context.window_manager.cycle = self.cycle
...
def buildBipedSkeleton():
    ...
    u.strName = setName('biped', n)
    mod = math.fmod(n, 2)
    if(mod == 0):
        x = 1.0 * n - mod    # Can you build an armature at the origin
    else:                    # then start building bones independent of it?
        x = 1.0 * -n - mod
    y = -.4 * n; z = u.bipedHeight  # y = Head of handle (Armature point)
    # at is named u.strName_at or u.strName_at.00#  
    at = buildRootArmature(biped, u.strName, x, y, z) # creates point, no bone yet
    biped.armature = at
    # %%% FIRST BONE %%% FIRST CHARACTER WILL E NAMED rgbiped01
    VHead = [0, 0, 0]
    Vtail = [0, -.3, 0] # Tail of Handle  
    bone = setHandle(at, u.strName, VHead, Vtail) 
    ...
    #angle.parent = at.edit_bones[u.strName + '_bone']
    V_hipC = (0, .1, -.1)
...
class BipedFns(object):
    ...
    def setBipedWalk(self, context):
        ...
        u.getSelectedCharacterName()
        #
		# See scratch2 for attempt at fixing this x-y axis
		# and z-rotation issue
		z = radians(bpy.data.objects[u.strName].pose.bones[u.strName + '_bone'].rotation_euler.z)
        fnx = "setHorizontalSpeed() * cos(" + str(z) + ")"
        setAxisDriver(getEuler(u.strName+'_bone'), fnx, 1, 'location')
        fny = "setHorizontalYSpeed() * sin(" + str(z) + ")"
        setAxisDriver(getEuler(u.strName+'_bone'), fny, 0, 'location')
		#
        ...
        #
        RP = "+" + str(.1)
        fnL = "(" + 'clock()' + RP + ")" + u.str0AtFrame0
        setDriver(getEuler('femurJ1.L'), fnL, 0)
        fnR = "(-1*" + 'clock()' + RP + ")" + u.str0AtFrame0
        setDriver(getEuler('femurJ1.R'), fnR, 0)
        # tibiaJ1
        ...
        RP = "-" + str(.4)
        fnL = "(" + 'clock()' + RP + ")" + u.str0AtFrame0
        setAxisDriver(getEuler('tibiaJ1.L'), fnL, 0)
        fnR = "(-1*" + 'clock()' + RP + ")" + u.str0AtFrame0
        setAxisDriver(getEuler('tibiaJ1.R'), fnR, 0)
        # Ankles
        ...
        RP = "+" + str(.3)
        fnL = "(-1*" + 'clock()' + RP + ")" + u.str0AtFrame0
        setAxisDriver(getEuler('ankle.L'), fnL, 0)
        fnR = "(" + 'clock()' + RP + ")" + u.str0AtFrame0
        setAxisDriver(getEuler('ankle.R'), fnR, 0)
        # Toes
        ...
        RP = "+" + str(.1)
        fnL = "(-1*" + 'clock()' + RP + ")" + u.str0AtFrame0
        setAxisDriver(getEuler('toe.L'), fnL + u.str0AtFrame0, 0)
        fnR = "(" + 'clock()' + RP + ")" + u.str0AtFrame0
        setAxisDriver(getEuler('toe.R'), fnR + u.str0AtFrame0, 0)
        #
        # Upper Back
        RP = "*" + str(.4)
        fn = "(" + 'clock()' + RP + ")" + u.str0AtFrame0
        bBackJ4Driver = setDriver(getEuler('bBackJ4'), fn, 1)
        #
        # Compensate for shoulder r0tate by rotating neck and head in opposite direction, in three parts
        RR = "*" + str(.13)
        fn = "(-1*" + 'clock()' + RR + ")" + u.str0AtFrame0
        neckJ2Driver = setDriver(getEuler('neckJ2'), fn, 1)
        neckJ3Driver = setDriver(getEuler('neckJ3'), fn, 1)
        neckJ4Driver = setDriver(getEuler('neckJ4'), fn, 1)
        # Arms rotation
        if(hasattr(bpy.types.WindowManager, "armRotation")):
            armRotation = str(bpy.context.window_manager.armRotation * .2)
        RR = "*" + str(1)
        fnL = "(" + 'clock()' + RR + ")" + u.str0AtFrame0
        setAxisDriver(getEuler('armJ1.L'), fnL, 0)
        fnR = "(-1*" + 'clock()' + RR + ")" + u.str0AtFrame0
        setAxisDriver(getEuler('armJ1.R'), fnR, 0)
        # Elbows
        RP = "-" + str(1.4)
        RR = "*" + str(.2)
        fnL = "(" + 'clock()' + RP + RR + ")" + u.str0AtFrame0
        setAxisDriver(getEuler('armJ3.L'), fnL, 0)
        fnR = "(-1*" + 'clock()' + RP + RR + ")" + u.str0AtFrame0
    ...
    def setSwayLR(self, context):  # left - right sway movement
        ...
        fn = "-(asin(" + 'clock()' + ")* " + u.swayLR + "*.01)"
        # bBackJ1
        setDriver(getEuler('bBackJ1'), fn, 2)
        bpy.ops.object.mode_set(mode='OBJECT')
        #
    def setSwayFB(self, context):  # forward - backward sway movement
        ...
        fn = "-(asin(" + 'clock()' + ")* " + u.swayFB + "*.01)"
        setDriver(getEuler('bBackJ1'), fn, 0)
        bpy.ops.object.mode_set(mode='OBJECT')
        #
    def setBounce(self, context):  # Bounce
        ...
        eqBounce = "-(asin(" + 'clock()' + ")* " + u.bounce + "*.01)"
        dr_Bounce = setDriver(getEuler(u.strName + '_bone'), eqBounce, 2, 'location') # Bounce
        bpy.ops.object.mode_set(mode='OBJECT')
        #        
    def setLegSpread(self, context): # Leg Spread
        ...
        dr1 = setDriver(getEuler('femurJ1.L'), str(-u.legSpread), 2)
        dr2 = setDriver(getEuler('femurJ1.R'), str(u.legSpread), 2)
        bpy.ops.object.mode_set(mode='OBJECT')
        #
    def setLegArch(self, context):
        ...
        fn = u.legArch
        setDriver(getEuler('femurJ1.L'), str(-fn), 1)
        setDriver(getEuler('femurJ1.R'), str(fn), 1)
        bpy.ops.object.mode_set(mode='OBJECT')
        #
    ...
    def setHead(self, context):
        ...
        fn = "-(asin(" + 'clock()' + ") * .2)/3.14 + " + u.headTurn
    ...
    def setShoulder(self, context):
        ...
        # Upper Back
        fn = "(asin(" + 'clock()' + ") * " + shoulderRotate + ")/3.14"
        bBackJ4Driver = setDriver(getEuler('bBackJ4'), fn, 1)
        # Compensate for shoulder r0tate by rotating neck and head in opposite direction, in three parts
        fn = "-(asin(" + 'clock()' + ") * " + u.shoulderRotate + "/3)/3.14"
        neckJ2Driver = setDriver(getEuler('neckJ2'), fn, 1)
        neckJ3Driver = setDriver(getEuler('neckJ3'), fn, 1)
        neckJ4Driver = setDriver(getEuler('neckJ4'), fn, 1)
        ...
        fn = "(asin(" + 'clock()' + ") * " + u.shoulderUD + ")/3.14"
        setDriver(getEuler('shoulder.L'), fn, 0)
        fn = "-(asin(" + 'clock()' + ") * " + u.shoulderUD + ")/3.14"
        setDriver(getEuler('shoulder.R'), fn, 0)
        bpy.ops.object.mode_set(mode='OBJECT')
        #
    def setHip(self, context):
        ...
        fn = "(asin(" + 'clock()' + ") * " + u.hipSway + ")/3.14"
        setDriver(getEuler('pelvis'), fn, 2) # was 0
        # Pelvis / Hip rotation    
        if(hasattr(bpy.types.WindowManager, "hipRotate")):
            u.hipRotate = str(bpy.context.window_manager.hipRotate*.1)
        fn = "(asin(" + 'clock()' + ") * " + u.hipRotate + ")/3.14"
        setDriver(getEuler('pelvis'), fn, 1)
        # hip up - down movement
        if(hasattr(bpy.types.WindowManager, "hipUD")):
            u.hipUD = str(bpy.context.window_manager.hipUD*.06)
        fn = "(asin(" + 'clock()' + ") * " + u.hipUD + ")/3.14"
        setDriver(getEuler('hip.L'), fn, 0)
        fn = "-(asin(" + 'clock()' + ") * " + u.hipUD + ")/3.14"
        setDriver(getEuler('hip.R'), fn, 0)
        bpy.ops.object.mode_set(mode='OBJECT')
        ob = bpy.data.objects.get(u.strName)
